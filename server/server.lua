...
-- Ers Gift Box Useable Item Event --
QBCore.Functions.CreateUseableItem("giftbox", function(source, item)
    ...
    if not Player.Functions.RemoveItem('giftbox') then return end
        TriggerClientEvent('Ers-GiftBox:Client:OpenGiftBox', src, item)
end)

-- Ers Gift Box Rewards --
RegisterNetEvent('Ers-GiftBox:Server:GiftBoxRewards', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
